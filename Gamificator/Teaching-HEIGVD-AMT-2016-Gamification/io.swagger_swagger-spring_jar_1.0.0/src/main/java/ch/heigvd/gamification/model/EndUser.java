...
@NamedQueries({
    @NamedQuery(name = "EndUser.getBestUsers", query = "SELECT e, p.pointScale, SUM(p.point)FROM EndUser e, PointAwards p WHERE e.app = :app AND p.enduser = e GROUP BY p.enduser.id, p.pointScale.id ORDER BY SUM(p.point) DESC"),
